...
const std::string clientPath = "../../../../ClientStorage";
...
bool Sender::sendFile(Message msg)
{
  ...
  }else if (msg.value("type") == "CheckIn")
  {
	  std::size_t pos = path.find_last_of("/");
	  std::string v = path.substr(pos + 1);
	  fileSpec = clientPath + "/" + v + "/" + msg.file();
  }
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	if (msg.value("type") == "CheckOut")
	{
		...
		std::size_t pos = path.find_last_of("/");
		std::string v = path.substr(pos + 1);
		fileSpec = clientPath + "/" + v + "/" + fileName;
	}
	else if (msg.value("type") == "CheckIn")
	{
		fileSpec = sendFilePath + "/" + fileName;
	}
